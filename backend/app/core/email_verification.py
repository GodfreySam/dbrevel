...
class EmailVerificationStore:
    """Store for managing email verification OTPs."""
    ...
    async def create_verification_otp(
        ...
        Args:
            user_id: User ID
            email: User email
            expires_in_minutes: OTP expiration time in minutes (default 30)
    ...
    async def invalidate_user_otps(self, user_id: str) -> int:
        """
        Invalidate all verification OTPs for a user.
...
# Global email verification store instance
email_verification_store: Optional[EmailVerificationStore] = None


def init_email_verification_store(mongo_url: str, db_name: str = "dbrevel_platform"):
    """Initialize the global email verification store."""
    global email_verification_store
    email_verification_store = EmailVerificationStore(mongo_url, db_name)
    return email_verification_store
