...
class DatabaseConfig(BaseModel):
    ...
    type: str = Field(
        ..., description="Database type (postgres, mongodb, mysql, redis, etc.)"
    )
...
class ProjectCreateRequest(BaseModel):
    ...
    databases: Optional[List[Dict[str, str]]] = Field(
        default=None,
        description="List of database configurations (new format: [{'type': 'postgres', 'connection_url': '...'}, ...])",
    )
...
class ProjectConnectionTestRequest(BaseModel):
    ...
    project_id: Optional[str] = Field(
        default=None,
        description="Project ID to test connections for (uses project's saved URLs)",
    )
    postgres_url: Optional[str] = Field(
        default=None,
        description="PostgreSQL connection URL to test (if not using project_id)",
    )
    mongodb_url: Optional[str] = Field(
        default=None,
        description="MongoDB connection URL to test (if not using project_id)",
    )
